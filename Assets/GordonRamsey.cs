...
public class GordonRamsey : MonoBehaviour
{
        ...
        if (_target)
        {
            switch (STATE)
            {
                case RAMSEY_STATE.FOOD_DROPPED:
                    currentState = StartCoroutine(DroppedFoodAttack(_target));
                    break;
                case RAMSEY_STATE.FOOD_BURNT:
                    currentState = StartCoroutine(BurntFoodAttack(_target));
                    break;
                case RAMSEY_STATE.FOOD_RAW:
                    currentState = StartCoroutine(RawFoodAttack(_target));
                    break;
                case RAMSEY_STATE.FOOD_TRASH:
                    currentState = StartCoroutine(TrashedFoodAttack(_target));
                    break;
            }
        }
    ...
    private IEnumerator DroppedFoodAttack(Transform target)
    {
        ...
        playerSpeed = _target.GetComponent<PlayerController>().movementSpeed;
        ...
        _target.GetComponent<PlayerController>().movementSpeed = playerSpeed;
    ...
    private IEnumerator TrashedFoodAttack(Transform target)
    {
        ...
        playerSpeed = _target.GetComponent<PlayerController>().movementSpeed;
        _target.GetComponent<PlayerController>().movementSpeed = 0;
